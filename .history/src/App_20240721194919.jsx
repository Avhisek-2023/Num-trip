...
const App = () => {
  ...
  return loading ? (
    ...
    <div className="container flex flex-col items-center justify-center w-full h-auto my-40">
      <div className="px-4 py-4 border-4 border-cyan-800 rounded-xl">
        {twoDValue.map((row, i) => (
          <div className="flex w-1/2 gap-4 lg:w-1/4 " key={i}>
            {row.map((e, j) => (
              <div
                className="px-6 py-4 text-xl font-bold text-center transition duration-300 ease-in-out bg-blue-100 border-2 border-blue-900 cursor-pointer hover:bg-indigo-200"
                key={j}
