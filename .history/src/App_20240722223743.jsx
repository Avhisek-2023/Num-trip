...
const App = () => {
  ...
  const getRandomColor = () => {
    ...
    return color.slice(1); // Remove the '#' symbol
  };
          ...
          <div className="flex w-1/2 lg:w-full " key={i}>
            {row.map((e, j) => (
              <div
                className={
                  nameClass(e) +
                  " w-12 h-12 py-2 m-1 text-xl font-bold text-center transition duration-500 ease-in-out bg-blue-100 border-2 border-blue-900 rounded-full cursor-pointer hover:bg-indigo-200"
                }
                key={j}
