...
const App = () => {
  ...
  useEffect(() => {
    if (storeIndex.length > 0) {
      ...
      for (let i = 0; i < 3; i++) {
        if (twoDValue[storeHit.row][storeHit.col] == chosenArray[i]) {
          chosenArrayIndex = i;
        }
      }
      console.log(chosenArrayIndex);
      console.log(chosenArray[Math.floor(Math.random() )] chosenArrayIndex);
