...
const App = () => {
  ...
  useEffect(() => {
    if (storeIndex.length > 0) {
      ...
      //   console.log(move);
      //   if (move) {
      //     alert("You have moves to continue.");
      //   } else {
      //     storeHighest();
      //     setModal(true);
      //     // console.log(highest);
  ...
  const moves = () => {
    if (!twoDValue || twoDValue.length === 0) return; // Add this check
    let x = 0;
    for (let i = 0; i < 5; i++) {
      for (let j = 0; j < 5; j++) {
        if (j < 4 && twoDValue[i][j] === twoDValue[i][j + 1]) {
          setMove(() => true);
          x = 1;
        }
        if (i < 4 && twoDValue[i][j] === twoDValue[i + 1][j]) {
          setMove(() => true);
          x = 1;
        }
      }
