...
const App = () => {
  ...
  const checkChange = () => {
    let key;
    twoDValue.forEach((rows, i) => {
      rows.forEach((e) => {
        if (e !== twoDValue[storeHit.row][storeHit.col] && key) {
          console.log("Check Index");
        }
      });
    });
    console.log("Update mini arry");
  };
  return loading ? (
    <>Loading..</>
  ) : (
      ...
      <div className="p-2 border-4 shadow-lg shadow-inner shadow-cyan-900 border-cyan-800 bg-slate-300 rounded-xl">
        {twoDValue.map((row, i) => (
          <div className="flex w-1/2 lg:w-1/4 " key={i}>
            {row.map((e, j) => (
              <div
                className="px-6 py-4 m-1 text-xl font-bold text-center transition duration-300 ease-in-out bg-blue-100 border-2 border-blue-900 rounded-full cursor-pointer hover:bg-indigo-200"
                key={j}
